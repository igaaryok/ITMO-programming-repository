...
int create(FILE *fileArc, FILE *fileAdd, const char *fileName) {
    ...
    char sizeOfName[sizeof(int)];
    sprintf(sizeOfName, "%d", sizeof(fileName));
    fwrite(sizeOfName, sizeof(int), 1, fileArc);
    fwrite(fileName, sizeof(fileName)*2, 1, fileArc);
    fwrite(&fileSize, sizeof(int), 1, fileArc);
    fseek(fileAdd, 0, SEEK_SET);
...
int extract(FILE *fileArc, const char *arcName) {
    ...
    while (!feof(fileArc)) {
        ...
        int fileSize;
        FILE *outputFile;
        char fileNameSize[sizeof(int)];
        fread(fileNameSize, sizeof(int), 1, fileArc);
        fread(fileName, atoi(fileNameSize)*2, 1, fileArc);
